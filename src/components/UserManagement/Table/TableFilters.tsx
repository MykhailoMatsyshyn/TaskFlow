 ...
 *
 * This component provides filtering inputs for table columns.
 * It syncs local input state with the global `zustand` filter store
 * and applies a debounce to optimize filtering performance.
 *
 * Features:
 * - Supports text, number, and select filters based on `column.id`.
 * - Synchronizes with global filters in `useUsersFilterStore`.
 * - Handles dark mode styles dynamically.
 */
...
function TableFilters<TData, TValue>({
  ...
  const { filters, setFilter } = useUsersFilterStore();
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      setFilter(column.id as keyof UserFilters, inputValue ?? "");
    }, 300);
